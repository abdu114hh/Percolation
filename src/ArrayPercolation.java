...
public class ArrayPercolation implements Percolation {
    ...
    private int openSites; // instance variable to keep track of open sites
    private final boolean[][] open; // for creating a boolean list open
    ...
    public boolean isOpen(int i, int j) {
        // throw an exception if i or j is out of bounds
        if (i < 0 || i > open.length - 1 || j < 0 || j > open.length - 1) {
            throw new IndexOutOfBoundsException("Illegal i or j");
        }
    ...
    public boolean percolates() {
        // if there is a full site in the last row, return true
        for (int k = 0; k < n; k++) {
            if (isFull(n - 1, k)) {
                return true;
            }
    ...
    private void floodFill(boolean[][] full, int i, int j) {
        // return if i or j is out of bounds, or site(i,j) is not open
        if (i < 0 || i > n-1 || j < 0 || j > n-1 || !open[i][j]
            || full[i][j]) {
            return;
        }
        // full site (i, j)
        full[i][j] = true;
        // recursively call floodFill on east, west, north and south
